# -*- coding: utf-8 -*-
"""
Created on Fri Feb 14 11:04:15 2020
@author: Eduardo
"""
import numpy as np
class PFCM:
    def pfcm(self, data, c, expo=2, max_iter=1000, min_impro=0.005, a=1, b=4, nc=3):
        """
        ...
        for i in range(max_iter):
            current_cntr = cntr
            U, T, cntr, obj_fcn[i], ni = self.pstepfcm(data, cntr, U, T, expo, a, b, nc, ni)
            if i > 1:
    ...
    def pstepfcm(self, data, cntr, U, T, expo, a, b, nc, ni):
        mf = np.power(U, expo)
        tf = np.power(T, nc)
        tfo = np.power((1-T), nc)
        cntr = (np.dot(a*mf+b*tf, data).T/np.sum(
            a*mf+b*tf, axis=1).T).T
        dist = self.pdistfcm(cntr, data)
    ...
    def initf(self, c, data_n):
        A = np.random.random(size=(c, data_n))
        col_sum = np.sum(A, axis=0)
        return A/col_sum
    def pdistfcm(self, cntr, data):
        ...
        return out
    def pfcm_predict(self, data, cntr, expo=2, a=1, b=4, nc=3):
        """
